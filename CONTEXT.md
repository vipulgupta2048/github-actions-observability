...
### 6. **Dashboard Metric Naming Crisis (Round 1)** 🚨→✅
...
### 7. **Dashboard Metric Naming Crisis (Round 2)** 🚨→✅
**Problem**: After fixing attribute mapping, dashboards STILL showed no data despite metrics flowing correctly
**Root Cause**: **REVERSE DISCOVERY** - The dashboards were actually correct initially, but we "fixed" them wrong!

#### **The Plot Twist - Reality Check:**
```bash
...
github_actions_traces_span_metrics_duration_seconds_sum
github_actions_traces_span_metrics_duration_seconds_count
github_actions_traces_span_metrics_duration_seconds_bucket
```

#### **Critical Realization:**
```promql
# ❌ WRONG - What we "corrected" dashboards to use (Round 1)
...
#### **The Full Fix - Dashboard Queries Updated:**
1. **All Template Variables** - Updated to use correct metric names:
   ```promql
...
2. **All Panel Queries** - Fixed in both dashboards:
   ```promql
   # Workflow Exploration Dashboard - 20+ queries updated
   sum by (workflow_step, step_status) (github_actions_traces_span_metrics_calls_total{...})
   
   # Traces Detailed Dashboard - 11+ queries updated  
   sum by (workflow_step) (rate(github_actions_traces_span_metrics_calls_total{...}[5m]))
   ```

3. **Duration Metrics** - Corrected format:
   ```promql
   # Fixed histogram queries
   histogram_quantile(0.95, sum by (workflow_step, le) (rate(github_actions_traces_span_metrics_duration_seconds_bucket{...}[5m])))
   ```

#### **Spanmetrics Connector Truth:**
```yaml
...
connectors:
  spanmetrics:
    ...
    dimensions:
      - name: workflow_step
      - name: step_status
# Result: github_actions_traces_span_metrics_calls_total (not github_actions_calls_total)
```

**Impact**: Both critical dashboards now fully functional with proper metric references!
...
### 8. **SpanMetrics Connector Deep Dive** 🔍→✅
...
### 9. **Dashboard Enhancement & Usability** 📊→✨
**Added Features:**
...
# 4. Test basic query
curl "http://localhost:9090/api/v1/query?query=github_actions_traces_span_metrics_calls_total"
```

### **Template Variables Not Populating**
```promql
# ✅ CORRECT queries for template variables (with full namespace)
label_values(github_actions_traces_span_metrics_calls_total, service_name)
label_values(github_actions_traces_span_metrics_calls_total{service_name=~"$service"}, workflow_step)  
...
### **🚨 CRITICAL: Collector Logs vs. Prometheus Metrics**
**DO NOT** rely on collector logs to determine current data quality! Here's why:
...
# Returns only current active metrics (should be 0 results if system is working)
```

**Why This Matters:**
- Spanmetrics connector caches spans for 30 minutes (`metrics_expiration: 30m`)
- Old spans processed before fixes may still appear in logs
...
### **💡 CRITICAL METRIC NAMING INSIGHT** 
...
### 10. **Attribute Mapping Crisis - OTTL Transform Processor** 🚨→✅
...
#### **Critical OTTL Syntax Discovery:**
```yaml
...
#### **Key OTTL Learnings:**
1. **Path Expressions**: Use `span.attributes["key"]` not `attributes["key"]`
2. **Status Codes**: Only `STATUS_CODE_OK`, `STATUS_CODE_ERROR`, `STATUS_CODE_UNSET` exist (no CANCELLED)
3. **Context Inference**: Transform processor auto-infers context from statements - no explicit `context: span` needed
4. **Pipeline Configuration**: Remove obsolete `attributes` processor from pipeline when using transform
...
#### **Comprehensive Verification Results (2025-09-19 19:00 IST):**
**Fresh Diagnostic Session Confirmed:**
- ✅ **Complete Pipeline Debug**: 17 metrics discovered, 6 data points with proper mapping
- ✅ **Zero Unknown Values**: 0 metrics with `workflow_step="unknown"` or `step_status="unknown"`
- ✅ **Template Variables Working**: 6 distinct workflow steps, 2 step status values available
...
Execution Count: 24 total (20 success + 4 unset)
```

**Cache vs. Current Data Discovery:**
